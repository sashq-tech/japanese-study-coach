...
if (lessons.METADATA?.reviewStatus !== "needs_review") throw new Error("Kanji package must remain review-gated.");
if (lessons.METADATA?.compatibility?.website !== "hold") throw new Error("Unreviewed kanji must not be marked website-ready.");
...
for (const [index, lesson] of lessons.LESSONS.entries()) {
  ...
  for (const item of items) {
    ...
    if (item.reviewStatus !== "needs_review") throw new Error(`${item.id} must remain needs_review.`);
